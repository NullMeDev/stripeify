...
use colored::*;
use shopify_checker::{analyzer, checker};
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Test { gates, output, max_gates, random_amounts, min_amount, max_amount } => {
            run_checker(&gates, &output, max_gates, random_amounts, min_amount, max_amount)?;
        }
        Commands::Auto { 
            input, 
            max_analyze, 
...
fn run_checker(gates_file: &str, output_file: &str, max_gates: Option<usize>, random_amounts: bool, min_amount: f64, max_amount: f64) -> Result<()> {
    ...
    tokio::runtime::Runtime::new()?.block_on(async {
        checker::run_checker(gates_file, output_file, max_gates).await
    })?;
...
fn run_auto_mode(
    ...
    run_checker(temp_gates_file, output_file, max_test, random_amounts, min_amount, max_amount)?;
